...
package org.jraf.k2o.projects.scratcher
  ...
  rotate(y = 90.deg) {
    difference {
      ...
      translate(x = -outerRadius, y = outerRadius - thickness / 1.5) {
        Cube(x = outerRadius * 2, y = 4.mm, z = width)
      }
    }
  }
  // Extension
...
private fun Triangle(
  thickness: Length,
  width: Length,
  wristDiameter: Length,
  triangleDepth: Length,
) {
  hull {
    // Front
...
fun main() {
  openScad(
    SystemFileSystem.sink(Path(TMP_FOLDER, "scratcher.scad")).buffered(),
    fa = .1,
    fs = .1,
  ) {
    Scratcher()
